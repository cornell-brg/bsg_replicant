...
void brg_memcpy( void* dst, const unsigned char* src, size_t size ) {
...
#ifdef ARRAY
  uint_32t v[5];
  brg_memcpy( v, ctx->hash, 5 * sizeof( uint_32t ) );
#else
                       ...
                       sha1_ctx ctx[1] )
{
  ...
  {
    brg_memcpy( ( (unsigned char*)ctx->wbuf ) + pos, sp, space );
    sp += space;
    len -= space;
  ...
  brg_memcpy( ( (unsigned char*)ctx->wbuf ) + pos, sp, len );
}
